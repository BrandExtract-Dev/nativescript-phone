...
var CustomMFMessageComposeViewControllerDelegate = /** @class */ (function (_super) {
    ...
    CustomMFMessageComposeViewControllerDelegate.prototype.messageComposeViewControllerDidFinishWithResult = function (controller, result) {
    ...
    CustomMFMessageComposeViewControllerDelegate.ObjCProtocols = [MFMessageComposeViewControllerDelegate];
...
export function dial(telNum, prompt) {
    try {
        ...
        const phoneNumberEncode = encodeURIComponent(telNum);
        const url = NSURL.URLWithString(`${sURL}:${phoneNumberEncode}`);
         
        if (UIApplication.sharedApplication.canOpenURL(url)) {
            UIApplication.sharedApplication.openURLOptionsCompletionHandler(url, null, null);
            NSPhoneEventEmitter.notify({
